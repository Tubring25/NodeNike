...
class techniqueService {
  async getList () {
    try {
     let res = await techniqueModule.findAll()
  ...
  async addItem (data) {
    ...
    try {
      let hasOne = await techniqueModule.findAll({where: {[Op.or]: [{name: name}, {code: code}]}})
      console.log(hasOne)
      if (hasOne.length > 0 ) {
        return { code: 0, data: '不可重复添加'}
      }
      techniqueModule.create({name: name, code: code})
  ...
  async editItem (data) {
    ...
    try {
      ...
      techniqueModule.update({name: name, code: code}, {where:{id:id}})
