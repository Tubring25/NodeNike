<template>
  <div class="Special-attribute-container">
...
<script>
import { getSpecialList, addSpecial, deleteSpecial, editSpecial } from '@/api/goods'
export default {
  data() {
    return {
      cloLoading: false,
  ...
  methods: {
    ...
    confirm(type, item){
      ...
      if(item.id) {
        editSpecial({id: item.id, name: item.name.trim(), type: type.toString()}).then(res=>{
        ...
        addSpecial({name: item.name.trim(), type: type.toString()}).then(res=>{
          type == 0 ? this.cloLoading = false : this.ShoeLoading = false
          if(res.code == 1){
            this.getList_()
          }
    ...
    close(item){
      this.$confirm('确认删除'+item.name+'吗?', '', {
        ...
        deleteSpecial({id: item.id}).then(res=>{
          if(res.code == 1) {
