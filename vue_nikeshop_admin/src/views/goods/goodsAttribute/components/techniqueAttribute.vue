<template>
  <div class="Technique-attribute-container">
...
<script>
import { getTechniqueList, addTechnique, deleteTechnique, editTechnique } from '@/api/goods'
export default {
  data() {
    return {
      cloLoading: false,
  ...
  methods: {
    ...
    close(item){
      this.$confirm('确认删除'+item.name+'吗?', '', {
        ...
        deleteTechnique({id: item.id}).then(res=>{
          if(res.code == 1) {
