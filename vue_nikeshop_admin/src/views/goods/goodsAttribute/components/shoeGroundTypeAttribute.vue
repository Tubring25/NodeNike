...
<script>
import { getShoeGroundType, addShoeGroundType, deleteShoeGroundType, editShoeGroundType } from '@/api/goods'
export default {
  data() {
    return {
      shoeLoading: false,
  ...
  methods: {
    ...
    confirm(item){
      ...
      if(item.id) {
        editShoeGroundType({id: item.id, name: item.name.trim()}).then(res=>{
          if(res.code == 1) {
            this.getList_()
          }
        })
      } else {
        addShoeGroundType({name: item.name.trim()}).then(res=>{
          if(res.code == 1){
            this.getList_()
          }
        })
    ...
    close(item){
      this.$confirm('确认删除'+item.name+'吗?', '', {
        ...
        deleteShoeGroundType({id: item.id}).then(res=>{
          if(res.code == 1) {
